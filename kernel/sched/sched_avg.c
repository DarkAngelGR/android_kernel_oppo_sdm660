...
#ifdef CONFIG_VENDOR_REALME
//xiaocheng.li@Swdp.shanghai, 2016/4/19, Add error handling to avoid BUG_ON()
	tmp_avg = (int)DIV64_U64_ROUNDUP(tmp_avg, diff);
	tmp_big_avg = (int)DIV64_U64_ROUNDUP(tmp_big_avg, diff);
	tmp_iowait = (int)DIV64_U64_ROUNDUP(tmp_iowait, diff);
	...
	*iowait_avg = tmp_iowait > INT_MAX ? 0 : (int)tmp_iowait;
	...
	trace_sched_get_nr_running_avg(*avg, *big_avg, *iowait_avg, *max_nr, *big_max_nr);
#else
