 ...
 **  Ziqing.Guo               2018/07/16                  remove notify momentarily to fix the dump caused by notify
 **  Ziqing.Guo               2018/07/31                  add to avoid null pointer problem
 ***********************************************************************************/
                             ...
                             unsigned long event, void *data)
{
    ...
    int retval = 0;
	LOG_MSG_DEBUG(INFO_LOG, "[%s] silfp_fb_callback event=%zu", __func__, event);
    ...
    if (event == MSM_DRM_ONSCREENFINGERPRINT_EVENT) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "[%s] UI ready enter op_mode: 0x%x", __func__, op_mode);
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    fp_dev->resource_is_init = 0;
    fp_dev->power_is_off = 1;
    if ((boot_mode == MSM_BOOT_MODE__FACTORY) && (FP_SILEAD_OPTICAL_70 == get_fpsensor_type())) {
        if (fp_dev->resource_is_init == 0) {
            silfp_parse_dts(fp_dev);
            fp_dev->resource_is_init = 1;
        }
        if(fp_dev->power_is_off == 1) {
            silfp_hw_poweron(fp_dev);
