...
#ifdef CONFIG_VENDOR_REALME
/*Jindian.Guan@Cam.Drv, 2018/3/5, add for [tick sound while dual cam af use same ldo]*/
static int32_t gpio1_cnt = 0;
#endif
	...
	if (a_ctrl->actuator_state != ACT_DISABLE_STATE) {
		...
		for (gpio = SENSOR_GPIO_AF_PWDM;
			...
			if (a_ctrl->gconf &&
				...
				#ifdef CONFIG_VENDOR_REALME
				/*Jindian.Guan@Cam.Drv, 2018/3/5, add for [tick sound while dual cam af use same ldo]*/
				if (a_ctrl->gconf->gpio_num_info->gpio_num[gpio] == 1) {
					gpio1_cnt--;
				}
				if (gpio1_cnt > 0 && a_ctrl->gconf->gpio_num_info->gpio_num[gpio] == 1) {
					CDBG("%s gpio1 not pulldown return %d,cnt:%d\n",__func__, __LINE__,gpio1_cnt);
					kfree(a_ctrl->step_position_table);
					a_ctrl->step_position_table = NULL;
...
static int32_t msm_actuator_power_up(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	for (gpio = SENSOR_GPIO_AF_PWDM; gpio < SENSOR_GPIO_MAX; gpio++) {
		if (a_ctrl->gconf &&
			...
			if (gpio1_cnt > 1 && a_ctrl->gconf->gpio_num_info->gpio_num[gpio] == 1) {
				CDBG("%s gpio1 not pullup return %d,cnt:%d\n",__func__, __LINE__,gpio1_cnt);
