...
int afe_dsm_setget_params(uint8_t *payload, int size, int dir,
		uint32_t dst_port, uint32_t mod_id, uint32_t param_id)
{
...
int afe_dsm_ramp_dn_cfg(uint8_t *payload, int delay_in_ms)
{
	...
	*(params + 7) = 1;
	...
	afe_dsm_setget_params(payload, 8*sizeof(uint32_t), 0, DSM_RX_PORT_ID,
			AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
...
{
	return afe_dsm_setget_params(payload, size, 0, DSM_RX_PORT_ID,
					AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
}

int afe_dsm_rx_get_params(uint8_t *payload, int size)
{
	return afe_dsm_setget_params(payload, size, 1, DSM_RX_PORT_ID,
					AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
}

int afe_dsm_set_calib(uint8_t* payload)
{
	return afe_dsm_setget_params(payload, sizeof(uint32_t)*3, 0, DSM_TX_PORT_ID,
					AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
}
...
int afe_dsm_pre_calib(uint8_t* payload)
{
	...
	*(params + 3) = 4; // mode 0: disable, 1: enable, 2: bypass and pilot tone, 4: pilot tone only
	afe_dsm_setget_params(payload, 4*sizeof(uint32_t), 0, DSM_RX_PORT_ID,
			AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
...
int afe_dsm_post_calib(uint8_t* payload)
{
	...
	*(params + 3) = 1; // mode 0: disable, 1: enable, 2: bypass and pilot tone, 4: pilot tone only
	return afe_dsm_setget_params(payload, 4*sizeof(uint32_t), 0, DSM_RX_PORT_ID,
					AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
}

int afe_dsm_get_calib(uint8_t* payload)
{
	return afe_dsm_setget_params(payload, sizeof(uint32_t)*14, 1, DSM_TX_PORT_ID,
				AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
}
...
int afe_dsm_get_average_calib(uint8_t* payload)
{
	...
	for (i = 0; i < 4; i++) {
		rc = afe_dsm_setget_params(payload, sizeof(uint32_t)*14, 1, DSM_TX_PORT_ID,
				AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
		if (rc != 0) {
			sum_rdc[0] = 0;
			sum_rdc[1] = 0;
...
{
	return afe_dsm_setget_params((int8_t*)payload, 4*100, 1, DSM_TX_PORT_ID,
				AFE_MODULE_DSM_TX, AFE_PARAM_ID_DSM_INFO);
}
#endif /* CONFIG_VENDOR_REALME */
